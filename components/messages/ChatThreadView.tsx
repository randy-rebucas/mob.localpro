...
export function ChatThreadView({ threadId, emptyHint }: Props) {
  ...
  const sendMutation = useMutation({
    mutationFn: (body: string) => messageService.postMessage(threadId, body),
    onSuccess: async () => {
      setDraft('');
      await qc.invalidateQueries({ queryKey: ['messages', 'thread', threadId] });
      await qc.invalidateQueries({ queryKey: ['messages', 'threads'] });
  ...
  const renderItem = useCallback(
    ({ item }: { item: ChatMessage }) => (
      <View className="mx-4 mb-3 rounded-2xl border border-neutral-200 bg-white px-3 py-2.5 dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <Text className="mt-1 text-[10px] text-neutral-400">{new Date(item.createdAt).toLocaleString()}</Text>
      </View>
    ),
    []
  );
